...
async def test_save_persists_memory_with_embedding(monkeypatch):
    ...
    mgr = MemoryManager()
    memory_id = await mgr.save(
        "user prefers python", category="preference", user_id="u1"
    )
...
async def test_vector_search_returns_rows(monkeypatch):
    ...
    fake_conn.fetch = AsyncMock(
        return_value=[{"id": "m1", "category": "fact", "content": "x"}]
    )
...
async def test_fulltext_search_returns_rows(monkeypatch):
    fake_conn = AsyncMock()
    fake_conn.fetch = AsyncMock(
        return_value=[{"id": "m2", "category": "fact", "content": "y"}]
    )
...
async def test_save_feedback_learning_promotes_positive_feedback(monkeypatch):
    ...
    result = await mgr.save_feedback_learning(
        "draft release notes", "up", "keep this concise tone", user_id="u1"
    )
...
async def test_save_feedback_learning_promotes_negative_feedback(monkeypatch):
    ...
    result = await mgr.save_feedback_learning(
        "answer billing question", "down", "too vague about pricing", user_id="u1"
    )
...
async def test_search_by_time_range_returns_rows(monkeypatch):
    fake_conn = AsyncMock()
    fake_conn.fetch = AsyncMock(
        return_value=[{"id": "m4", "category": "context", "content": "z"}]
    )
...
async def test_apply_relevance_decay_updates_rows(monkeypatch):
    ...
    mgr = MemoryManager()
    updated = await mgr.apply_relevance_decay(
        user_id="u1", half_life_days=90.0, min_relevance=0.1
    )
