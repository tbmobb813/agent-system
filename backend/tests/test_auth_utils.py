...
async def test_verify_api_key_accepts_master_key_bypass(monkeypatch):
    monkeypatch.setattr(
        "app.config.settings.BACKEND_API_KEY", "sk-agent-master-1,sk-agent-master-2"
    )
...
async def test_verify_api_key_accepts_active_db_key_and_schedules_last_used_update(
    monkeypatch,
):
    async def fake_fetchrow(_query, _key_hash):
