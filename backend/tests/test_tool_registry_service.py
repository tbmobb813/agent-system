...
async def test_tool_health_snapshot_includes_core_tools():
    reg = ToolRegistry()
    snap = await reg.tool_health_snapshot()
    names = {row["tool"] for row in snap}
    assert "web_search" in names
    assert "file_operations" in names
    assert "code_execution" in names
...
async def test_searxng_search_handles_connect_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.tools.tool_registry.httpx.AsyncClient", lambda timeout=15.0: _FailClient()
    )
...
async def test_browser_automation_blocks_host_not_in_allowlist(monkeypatch):
    ...
    nav = await registry._browser_automation(
...
async def test_browser_automation_success_actions(monkeypatch, tmp_path):
    ...
    ext = await registry._browser_automation(
        action="extract", url="https://example.com", selector=".item"
    )
    assert "one" in ext and "two" in ext

    shot_path = str(tmp_path / "shot.png")
    shot = await registry._browser_automation(
        action="screenshot", url="https://example.com", screenshot_path=shot_path
    )
    assert "Screenshot saved" in shot

    click = await registry._browser_automation(
        action="click", url="https://example.com", selector="#btn"
    )
    assert "Clicked: #btn" == click

    fill = await registry._browser_automation(
        action="fill", url="https://example.com", selector="#name", text="Ada"
    )
