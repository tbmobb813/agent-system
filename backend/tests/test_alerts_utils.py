...
async def test_webhook_handles_success_failure_and_exception(monkeypatch):
    mgr = AlertManager()
    monkeypatch.setattr(
        "app.config.settings.ALERT_WEBHOOK_URL", "https://example.com/hook"
    )
    ...
    monkeypatch.setattr(
        "app.utils.alerts.httpx.AsyncClient", lambda timeout=10.0: _ClientOK()
    )
    await mgr._webhook("warn", 8.0, 10.0, 80.0)

    monkeypatch.setattr(
        "app.utils.alerts.httpx.AsyncClient", lambda timeout=10.0: _ClientFail()
    )
    await mgr._webhook("warn", 8.0, 10.0, 80.0)

    monkeypatch.setattr(
        "app.utils.alerts.httpx.AsyncClient", lambda timeout=10.0: _ClientBoom()
    )
    await mgr._webhook("warn", 8.0, 10.0, 80.0)
